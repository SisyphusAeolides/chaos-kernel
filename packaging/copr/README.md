...
This directory contains the CIQ/RLC kernel packaging used to build Chaos
Kernel in COPR, alongside the retained legacy Fedora packaging.

The numbered patch series is the canonical kernel implementation. The CLK
6.18.43 RPM spec applies the reviewed CIQ port after the downstream source
...
The current CIQ package in `clk618/kernel-clk6.18.spec` targets the CIQ CLK
6.18.43 source layout. The older `clk612/` port remains available for CLK
6.12. Neither series is a universal patch for arbitrary
...
The CLK 6.18 build permanently compiles the bounded chaos math and enables
...
For the CIQ CLK 6.18 build, place
`linux-6.18.43-2.1.el10.tar.zst` beside the files in `clk618/`, then build
`clk618/kernel-clk6.18.spec` into a self-contained source RPM. The large
