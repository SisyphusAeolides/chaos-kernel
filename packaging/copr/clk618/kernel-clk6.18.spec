...
# RLC/EL10 and Fedora use the standard kmod capability. Older CIQ EL9
# environments retain the CIQ-specific capability used by the 6.12 port.
...
# Bundle bindgen-cli so the same source RPM remains buildable in EL9,
# Fedora, and EL10 chroots. EL10 uses its system package instead.
Source5000: bindgen-cli.tar.gz

# The CLK 6.18 Chaos Kernel port is applied in %prep after the CIQ source
# patches. Keep these numbered sources so the source RPM is self-contained and
# the patch runner can fail closed when the kernel layout changes.
...
Source6010: 0010-clk6.18-enable-full-chaos-feature-set.patch
...
%{log_msg "Apply bounded Chaos Kernel CLK 6.18 feature set"}
...
install -m 0644 %{SOURCE6010} "$chaos_patch_dir/0010-clk6.18-enable-full-chaos-feature-set.patch"
"$chaos_patch_dir/apply-chaos-patches.sh" \
  --apply --strict --series clk6.18 --patch-dir "$chaos_patch_dir" "$PWD"
