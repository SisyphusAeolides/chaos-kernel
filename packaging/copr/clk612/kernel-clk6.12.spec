...
# Keep the source-RPM package set stable when it is assembled in an EL host.
# Fedora-specific source declarations are intentionally not enabled here;
# Fedora chroots select their module map explicitly below.
...
%define tarfile_release %{specversion}-1%{source_buildid}.el%{el_version}
...
# CIQ's EL9 kmod package provides the ciq-kmod capability. RLC 10.2 is an
# EL10 platform and ships the standard kmod package instead, so keep the
# kernel installable there without introducing a CIQ-only runtime dependency.
%if 0%{?rhel} >= 10
%define chaos_kmod_requirement kmod
%else
%define chaos_kmod_requirement ciq-kmod
%endif
...
Requires: %{chaos_kmod_requirement}
...
# The CLK 6.12 Chaos Kernel port is applied in %prep after the CIQ source
# patches.  Keep these as numbered sources so the source RPM is self-contained
# and the patch runner can fail closed when the kernel layout changes.
Source6000: apply-chaos-patches.sh
Source6001: 0001-lib-add-bounded-fixed-point-chaos-math.patch
Source6003: 0003-mm-detect-nonlinear-free-page-divergence-at-OOM.patch
Source6006: 0006-tcp-add-bounded-Roessler-congestion-control.patch
Source6011: 0011-clk6.12-default-roessler.patch
Source6007: 0007-lib-add-Lorenz-Mandelbrot-and-Lyapunov-dynamics.patch
Source6010: 0010-clk6.12-enable-full-chaos-feature-set.patch
...
Summary: *.ipa-clones files generated by -fdump-ipa-clones for %{name}%{?1:-%{1}}\
...
Provides: installonlypkg(kernel)\
Provides: %{name}-%{?1:%{1}-}uname-r = %{KVERREL}%{uname_suffix %{?1:+%{1}}}\
Requires: %{name}%{?1:-%{1}}-modules-core-uname-r = %{KVERREL}%{uname_suffix %{?1:+%{1}}}\
Requires(pre): %{kernel_prereq}\
%if 0%{?rhel}\
...
%{log_msg "Apply bounded Chaos Kernel CLK 6.12 feature set"}
chaos_patch_dir=$(mktemp -d)
cleanup_chaos_patch_dir()
{
  rm -f "$chaos_patch_dir"/apply-chaos-patches.sh \
    "$chaos_patch_dir"/*.patch
  rmdir "$chaos_patch_dir"
}
trap cleanup_chaos_patch_dir EXIT
install -m 0755 %{SOURCE6000} "$chaos_patch_dir/apply-chaos-patches.sh"
install -m 0644 %{SOURCE6001} "$chaos_patch_dir/0001-lib-add-bounded-fixed-point-chaos-math.patch"
install -m 0644 %{SOURCE6003} "$chaos_patch_dir/0003-mm-detect-nonlinear-free-page-divergence-at-OOM.patch"
install -m 0644 %{SOURCE6006} "$chaos_patch_dir/0006-tcp-add-bounded-Roessler-congestion-control.patch"
install -m 0644 %{SOURCE6011} "$chaos_patch_dir/0011-clk6.12-default-roessler.patch"
install -m 0644 %{SOURCE6007} "$chaos_patch_dir/0007-lib-add-Lorenz-Mandelbrot-and-Lyapunov-dynamics.patch"
install -m 0644 %{SOURCE6010} "$chaos_patch_dir/0010-clk6.12-enable-full-chaos-feature-set.patch"
"$chaos_patch_dir/apply-chaos-patches.sh" \
  --apply --strict --series clk6.12 --patch-dir "$chaos_patch_dir" "$PWD"
...
%endif
	...
	linux.ciq_rocky,1,CIQ,linux,$KernelVer,mailto:secureboot@ciq.com
...
#
%changelog
* Mon Aug 31 2026 Sisyphus Aeolides <SisyphusAeolides@pm.me> - 6.12.104-1.chaos7
- Use the standard kmod capability on EL10 so the CLK kernel installs on RLC 10.2.
...
-- [CIQ] v6.12.101 - updated spec (github-actions[bot])
-- [CIQ] v6.12.99 - updated spec (github-actions[bot])
...
-- Linux 6.12.104 (https://cdn.kernel.org/pub/linux/kernel/v6.x/ChangeLog-6.12.104)
